...
|---|-------|-------|-------------|-----------|
| 1.1 | Unsaved Changes Indicator | S | Track dirty state when load order is mutated; show indicator on Save button and window title. Prevents silently losing work. | `AppState.swift`, `ModListView.swift`, `BG3MacModManagerApp.swift` |
| 1.2 | Sidebar Mod Count Badges | S | Add `.badge()` to sidebar items showing active+inactive mod count, profile count, and backup count. Data already in `appState`. | `ContentView.swift` |
| 1.3 | Keyboard Shortcuts | S | Add Cmd+E (export), Cmd+Backspace (deactivate selected), and other shortcuts to the menu bar commands block. Update HelpView. | `BG3MacModManagerApp.swift`, `HelpView.swift` |
...
| 1.8 | Persistent Mod Count in Status Bar | S | Show "N active / M inactive" in the bottom status bar as an always-visible summary. | `ContentView.swift` |
...
| 2.9 | Warnings Badge Outside Mods Tab | S | Show critical/warning count badge on "Mods" sidebar item so users see issues on other tabs. | `ContentView.swift` |
